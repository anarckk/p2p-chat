...
## PC端改造需求（2026-02-02 最新）

根据 2026-02-02 最新需求，对 PC 端进行以下改造：

### 一、聊天
- 用户列表优先显示用户名，若无用户名才显示 PeerID
- 新增用户聊天时不要有一个空消息
...
2. 网络数据日志显示的是 request-response 改造之后的网络日志，应有一个 request 字段和 response 字段，分别显示一个网络请求的 request 和 response
